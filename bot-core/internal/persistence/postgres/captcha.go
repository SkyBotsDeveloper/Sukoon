...
func (s *Store) SetCaptchaSettings(ctx context.Context, settings domain.CaptchaSettings) error {
	_, err := s.pool.Exec(ctx, `
		...
		SET enabled=$3, mode=$4, timeout_seconds=$5, failure_action=$6, challenge_digits=$7, updated_at=NOW()
		WHERE bot_id=$1 AND chat_id=$2
	`, settings.BotID, settings.ChatID, settings.Enabled, settings.Mode, settings.TimeoutSeconds, settings.FailureAction, settings.ChallengeDigits)
	return err
}

func (s *Store) CreateCaptchaChallenge(ctx context.Context, challenge domain.CaptchaChallenge) error {
	_, err := s.pool.Exec(ctx, `
		INSERT INTO captcha_challenges (id, bot_id, chat_id, user_id, prompt, answer, message_id, expires_at, status, failure_action)
		...
		DO UPDATE SET
			...
			created_at = NOW()
	`, challenge.ID, challenge.BotID, challenge.ChatID, challenge.UserID, challenge.Prompt, challenge.Answer, challenge.MessageID, challenge.ExpiresAt, challenge.FailureAction)
	return err
}
...
func (s *Store) GetPendingCaptchaChallenge(ctx context.Context, botID string, chatID int64, userID int64) (domain.CaptchaChallenge, error) {
	...
	err := s.pool.QueryRow(ctx, `
		SELECT id, bot_id, chat_id, user_id, prompt, answer, message_id, expires_at, status, failure_action
		FROM captcha_challenges
		WHERE bot_id=$1 AND chat_id=$2 AND user_id=$3 AND status='pending'
		ORDER BY created_at DESC
		LIMIT 1
	`, botID, chatID, userID).Scan(
		&challenge.ID, &challenge.BotID, &challenge.ChatID, &challenge.UserID, &challenge.Prompt, &challenge.Answer, &challenge.MessageID, &challenge.ExpiresAt, &challenge.Status, &challenge.FailureAction,
	)
...
func (s *Store) ListExpiredCaptchaChallenges(ctx context.Context, now time.Time, limit int) ([]domain.CaptchaChallenge, error) {
	...
	for rows.Next() {
		var challenge domain.CaptchaChallenge
		if err := rows.Scan(&challenge.ID, &challenge.BotID, &challenge.ChatID, &challenge.UserID, &challenge.Prompt, &challenge.Answer, &challenge.MessageID, &challenge.ExpiresAt, &challenge.Status, &challenge.FailureAction); err != nil {
			return nil, err
		}
