...
func (s *Store) LoadRuntimeBundle(ctx context.Context, botID string, chatID int64) (domain.RuntimeBundle, error) {
	...
	err := s.pool.QueryRow(ctx, `
		SELECT
			...
			cs.clean_commands, cs.clean_service_join, cs.clean_service_leave, cs.clean_service_pin, cs.clean_service_title, cs.clean_service_photo, cs.clean_service_other, cs.clean_service_videochat,
